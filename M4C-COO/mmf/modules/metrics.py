...
class TextVQAAccuracy(BaseMetric):
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        print("size check", batch_size, len(predictions))
...
@registry.register_metric("coo_prhmean")
class COO_PRHmean(BaseMetric):
    def __init__(self):
        super().__init__("coo_prhmean")
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        pred_answers = model_output["scores"].argmax(dim=-1)
        batch_size = sample_list.context_tokens.size(0)
        link_order_list = sample_list.link_order

        print(f"[size] batch:{batch_size}, question_id: {len(sample_list.question_id)}")
        print(f"pred_answers:{pred_answers.size()}, link_order: {len(link_order_list)}")

        gt_link_order_list = []
        for link_order in link_order_list:
            gt_link_order = []
            if link_order == []:
                continue
            else:
                for order in link_order[0].split():
                    if order == "<d>":
                        ...
                        gt_link_order.append(
                            str(int(order) + answer_space_size)
                        )  # For m4c_coo, answer_space_size = 5 (= # of special tokens). For m4c_answer answer_space_size = 11640
            gt_link_order_list.append(gt_link_order)
        ...
        for idx in range(batch_size):
            ...
            for answer_id in pred_answers[idx].tolist():
                if answer_id == answer_processor.EOS_IDX:
                    ...
                    pred_link.append(str(answer_id))
            pred_link_orders.append(pred_link)
        ...
        for idx, (pred, gt) in enumerate(zip(pred_link_orders, gt_link_order_list)):
            ...
            for pred_link in pred_link_list:
                if pred_link == "":
                    continue
                if pred_link in gt_link_list:
                    TP += 1  # true positive
                else:
                    FP += 1  # false positive

            for gt_link in gt_link_list:
                if gt_link == "":
                    continue
                if gt_link in pred_link_list:
                    continue  # already counted true positive
                else:
                    FN += 1  # false negative
        ...
        value = {
            "precision": torch.tensor(precision).to(sample_list.context_tokens.device),
            "recall": torch.tensor(recall).to(sample_list.context_tokens.device),
            "Hmean": torch.tensor(Hmean).to(sample_list.context_tokens.device),
        }

        print("value per gpu", value)
