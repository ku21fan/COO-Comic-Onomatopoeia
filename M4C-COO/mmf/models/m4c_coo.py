...
@registry.register_model("m4c_coo")
class M4C_COO(BaseModel):
    ...
    @classmethod
    def config_path(cls):
        return "configs/models/m4c_coo/defaults.yaml"
    ...
    def _build_obj_encoding(self):
        # for remove obj feat
        self.remove_obj_frcn = self.config.obj.get("remove_obj_frcn", False)
        self.remove_obj_bbox = self.config.obj.get("remove_obj_bbox", False)
        ...
        # when multi-gpu or distribute training, this part occur issue for some reason?
        if not self.remove_obj_frcn:
            # instead of fc7 1 layer we use 1 linear layer. = finetune 1 linear layer
            self.linear_obj_feat_finetune = nn.Linear(
                self.config.obj.mmt_in_dim, self.config.obj.mmt_in_dim
            )
            self.finetune_modules.append(
                {
                    "module": self.linear_obj_feat_finetune,
    ...
    def _build_ocr_encoding(self):
        ...
        if not self.remove_ocr_frcn:
            # instead of fc7 1 layer we use 1 linear layer. = finetune 1 linear layer
            self.linear_ocr_feat_finetune = nn.Linear(
                self.config.obj.mmt_in_dim, self.config.obj.mmt_in_dim,
            )
            self.finetune_modules.append(
                {
                    "module": self.linear_ocr_feat_finetune,
                    "lr_scale": self.config.lr_scale_frcn,
                }
            )
    ...
    def _forward_obj_encoding(self, sample_list, fwd_results):
        ...
        if self.remove_obj_frcn:
            obj_feat = torch.zeros_like(sample_list.image_feature_0)
        else:
            obj_feat_tmp = sample_list.image_feature_0
            obj_feat = self.linear_obj_feat_finetune(obj_feat_tmp)
            obj_feat = F.normalize(obj_feat, dim=-1)

        if self.remove_obj_bbox:
    ...
    def _forward_ocr_encoding(self, sample_list, fwd_results):
        ...
        # OCR PHOC feature (2648-dim)
        ocr_phoc = sample_list.context_feature_1
        ocr_phoc = F.normalize(ocr_phoc, dim=-1)
        assert ocr_phoc.size(-1) == 2648

        if self.remove_ocr_frcn:
            ocr_frcn = torch.zeros_like(sample_list.image_feature_0)
        else:
            ocr_frcn_tmp = sample_list.image_feature_0[:, : ocr_fasttext.size(1), :]
            ocr_frcn = self.linear_ocr_feat_finetune(ocr_frcn_tmp)
            ocr_frcn = F.normalize(ocr_frcn, dim=-1)
        ...
        ocr_feat = torch.cat([ocr_fasttext, ocr_phoc, ocr_frcn], dim=-1)
        ...
        # print("frcn", ocr_frcn_tmp.size(), torch.sum(ocr_frcn_tmp))
        # print("frcn_norm", ocr_frcn.size(), torch.sum(ocr_frcn))
        # print("bbox", ocr_bbox.size(), torch.sum(ocr_bbox))
    ...
    def _forward_output(self, sample_list, fwd_results):
        ...
        # print("s", scores.size(), fixed_scores.size(), dynamic_ocr_scores.size())
...
class MMT(BertPreTrainedModel):
    ...
    def forward(
        self, obj_emb, obj_mask, ocr_emb, ocr_mask, fixed_ans_emb, prev_inds,
    ):
