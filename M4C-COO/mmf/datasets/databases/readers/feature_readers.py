...
class FeatureReader:
    ...
    def _init_reader(self):
        if "_emb" in self.base_path:  # load COO data embedding
            self.feat_reader = COOFasterRCNNFeatureReader(self.max_features)
        # self.feat_reader = PaddedFasterRCNNFeatureReader(self.max_features)
        # Currently all lmdb features are with ndim == 2
        elif self.base_path.endswith(".lmdb"):
...
class COOFasterRCNNFeatureReader:
    ...
    def read(self, image_feat_path):
        ...
        # last part of "path" is "image_id"
        image_info["image_id"] = image_feat_path.split("/")[-1].split(".npy")[0]
        ...
        try:
            ...
            for bbox in image_info["bbox"]:
                ...
                tmp[1::2] /= height
                # print("a", tmp)
                normalized_bbox.append(tmp)

            image_info["ocr_normalized_boxes"] = np.array(
                normalized_bbox, dtype=np.float32
            )
            image_info["obj_normalized_boxes"] = np.array(
                normalized_bbox, dtype=np.float32
            )
            ...
            # print("empty image?")
            del image_info["features"]
            image_feature = torch.from_numpy(
                np.zeros([self.max_loc, 1024], dtype=np.float32)
            )
            image_info["ocr_normalized_boxes"] = np.zeros(
                [self.max_loc, 4], dtype=np.float32
            )
            image_info["obj_normalized_boxes"] = np.zeros(
                [self.max_loc, 4], dtype=np.float32
            )
            image_info["max_features"] = torch.tensor(0, dtype=torch.long)
