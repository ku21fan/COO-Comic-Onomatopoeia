...
class FastTextProcessor(VocabProcessor):
    ...
    def _try_download(self):
        ...
        #     model_file = os.path.join(get_mmf_env(key="data_dir"), model_file)

        print("fasttext model_file check", model_file)
...
@registry.register_processor("coo_phoc")
class COOPhocProcessor(VocabProcessor):
    """
    Compute PHOC features from text tokens  COO version
    """
...
@registry.register_processor("m4c_coo")
class M4CCOOProcessor(BaseProcessor):
    """
    Process a TextVQA answer for iterative decoding in M4C
    """
    ...
    def get_vocab_size(self):
        ...
        answer_vocab_nums += (
            self.max_length
        )  # the number of copy target is same with self.max_length
    ...
    def __call__(self, item):
        ...
        if answers == []:
            # print("empty link order")

            scores[0, self.EOS_IDX] = 1.0
            train_prev_inds = torch.zeros(self.max_copy_steps, dtype=torch.long)
            train_loss_mask = torch.zeros(self.max_copy_steps, dtype=torch.float)
            train_loss_mask[0] = 1.0
            idx_seq = ()
            ...
            for a in answers[0].split():
                if a == "<d>":
                    ...
                    link_order.append(self.EOS_IDX)
                    break
                else:
                    link_order.append(int(a) + 5)  # 5 is the number of special tokens

            all_idx_seq_list = [link_order]

            scores[0, link_order[0]] = 1.0
