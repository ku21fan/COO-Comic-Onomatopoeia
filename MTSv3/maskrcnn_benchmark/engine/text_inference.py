            ...
            if cfg.MODEL.SEG_ON:
                ...
                seg_results.append(
                    [
                        image_paths,
                        proposals,
                        seg_results_dict["rotated_boxes"],
                        seg_results_dict["polygons"],
                        seg_results_dict["preds"],
                        seg_results_dict["scores"],
                    ]
                )
                ...
                if predictions is not None:
                    if cfg.MODEL.CHAR_MASK_ON or cfg.SEQUENCE.SEQ_ON:
                        ...
                        global_predictions = [
                            o.to(cpu_device) for o in global_predictions
                        ]
                        results_dict.update(
                            {
                                image_paths[0]: [
                        ...
                        results_dict.update({image_paths[0]: [global_predictions[0],]})
            else:
...
def mask2polygon(mask, box, im_size, threshold=0.5, output_folder=None):
    ...
    if "bbox" not in output_folder and (
        "COO" in output_folder
        or "total_text" in output_folder
        or "cute80" in output_folder
    ):
...
def format_output(out_dir, boxes, img_name):
    with open(
        ...
        for i, box in enumerate(boxes):
            ...
            save_dict = {}
            if "bbox" not in out_dir and (
                "COO" in out_dir or "total_text" in out_dir or "cute80" in out_dir
            ):
...
def format_seg_output(
    results_dir, rotated_boxes_this_image, polygons_this_image, scores, img_name, ratio
):
    height_ratio, width_ratio = ratio
    with open(
        os.path.join(results_dir, "res_" + img_name.split(".")[0] + ".txt"), "wt"
    ) as res:
        if "bbox" not in results_dir and (
            "COO" in results_dir
            or "total_text" in results_dir
            or "cute80" in results_dir
        ):
...
def visualization(
    image, polygons, resize_ratio, colors, char_polygons=None, words=None
):
...
def vis_seg_map(
    image_path, seg_map, rotated_boxes, polygons_this_image, proposals, vis_dir
):
    ...
    seg_image = (
        Image.fromarray(
            (seg_map[0, : proposals.size[1], : proposals.size[0]] * 255).astype(
                np.uint8
            )
        )
        .convert("RGB")
    ...
    if "bbox" not in vis_dir and (
        "COO" in vis_dir or "total_text" in vis_dir or "cute80" in vis_dir
    ):
        ...
        for image_path, prediction in predictions.items():
            ...
            if cfg.MODEL.CHAR_MASK_ON or cfg.SEQUENCE.SEQ_ON:
                ...
                if char_mask is not None:
                    (
                        ...
                        rec_char_scoress,
                        char_polygons,
                    ) = process_char_mask(char_mask, boxes_char)
            else:
            ...
            for k, box in enumerate(boxes):
                ...
                if not cfg.MODEL.SEG.USE_SEG_POLY:
                    ...
                    if "bbox" in output_folder or not (
                        "COO" in output_folder
                        or "total_text" in output_folder
                        or "cute80" in output_folder
                    ):
            ...
            if vis:
                ...
                if cfg.MODEL.CHAR_MASK_ON:
                    visualization(
                        img, polygons, resize_ratio, colors, char_polygons, words
                    )
                else:
...
def inference(
    ...
    predictions, seg_predictions = compute_on_dataset(model, data_loader, device, cfg)
    # wait for all processes to complete before measuring the time
    # synchronize() # use only 1 gpu!
