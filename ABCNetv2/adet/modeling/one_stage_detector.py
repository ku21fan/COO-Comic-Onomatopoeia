...
def detector_postprocess(results, output_height, output_width, mask_threshold=0.5):
    ...
    if results.has("top_feat"):  # changed here for COO
        beziers = results.top_feat
...
class OneStageRCNN(GeneralizedRCNN):
    ...
    def __init__(self, cfg):
        ...
        nn.Module.__init__(
            self
        )  # To set roi_head as "None", we init from the GeneralizedRCNN
        self.backbone = build_backbone(cfg)
        self.proposal_generator = build_proposal_generator(
            cfg, self.backbone.output_shape()
        )
        self.roi_heads = (
            None  # To set roi_head as "None", we init from the GeneralizedRCNN
        )
    ...
    def forward(self, batched_inputs):
        ...
        # To set ABCNet detection only mode
        ...
        losses = {}
        # losses.update(detector_losses) # To set ABCNet detection only mode
    ...
    def inference(self, batched_inputs, detected_instances=None, do_postprocess=True):
        ...
        if detected_instances is None:
            ...
            results = proposals  # To set ABCNet detection only mode
        else:
            detected_instances = [x.to(self.device) for x in detected_instances]
            # results = self.roi_heads.forward_with_given_boxes(
            #     features, detected_instances
            # )
            results = detected_instances  # To set ABCNet detection only mode
